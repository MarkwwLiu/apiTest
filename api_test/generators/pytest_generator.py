...
from ..core.api_parser import ApiTestConfig
...
def http():
    executor = HttpExecutor(BASE_URL, DEFAULT_HEADERS)
    yield executor
    executor.close()
...
{% endif %}
    ...
    result_{{ step_idx }} = http.execute(
        ...
        headers=ep_{{ step_idx }}.get("headers", {}),
...
def generate_tests(
    ...
    if config.http_endpoints:
        ...
        content = HTTP_TEST_TEMPLATE.render(config=config)
    ...
    if config.wss_endpoints:
        ...
        content = WSS_TEST_TEMPLATE.render(config=config)
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        generated.append(path)
